...
def writeData(request):
    if request.method == 'POST':
        ...
        with connection.cursor() as cursor_1:
            cursor_1.execute("INSERT INTO car1(DeviceId,Command) VALUES ('" +
                             str(deviceID) + "','"+str(command) + "')")
            connection.commit()
            connection.close()
...
def writeStatus(request):
    if request.method == 'POST':
        ...
        with connection.cursor() as cursor_1:
            cursor_1.execute(
                "INSERT INTO carstatus(DeviceId,status) VALUES ('"+str(deviceID) + "','"+str(status) + "')")
            connection.commit()
            connection.close()
...
def updateStatus(request):
    if request.method == 'POST':
        ...
        status = request.POST.get("status", False)
        with connection.cursor() as cursor_1:
            cursor_1.execute("update carstatus set status='" +
                             str(status) + "' where DeviceId = '"+str(deviceID) + "'")
            connection.commit()
            connection.close()
